...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 28-01-2020 a las 13:00:57
...
--
-- Base de datos: `2aw3_reto4_azterketa`
--
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `spAllCaptcha` (IN `pId` INT)  NO SQL
SELECT * FROM captcha
WHERE captcha.id=pId$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spFindIdCountry` (IN `pCode` VARCHAR(10))  NO SQL
select * 
from country 
where pCode = country.Code$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spFindUserByUsername` (IN `pUser` VARCHAR(256))  NO SQL
BEGIN
SELECT users.*  FROM users 
WHERE users.username=pUser;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spLoginEncripted` (IN `pUser` VARCHAR(40))  NO SQL
BEGIN
SELECT users.idUser as idUser, users.password as pass, users.admin FROM users WHERE users.username=pUser;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spUsersCities` (IN `pIdUser` INT)  NO SQL
select *
from userscities
